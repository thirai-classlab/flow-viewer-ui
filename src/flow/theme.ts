...
export const SPLIT = {
  ...
  nestHeader: 22,
  /**
   * 左ペイン専用のノード間隔。共通の LAYOUT_GAP（node 30 / rank 64）より詰める。
   *
   * LogicFlow の実測: 5 階層の最深部で LAYOUT_GAP のままだと縮尺 0.53（11px の実効 5.8px）
   * まで落ちてラベルが潰れた。左ペインは幅 36〜52% しかないので、間隔を詰めるほうが可読性に効く。
   * #19（ELK 化）で 14/22 → 8/10 に詰めた。deepFlow の最深部（step-form）で
   * 1280x720 の縮尺が 0.79 → 0.85（下限ちょうど）に戻る（layout.ts の EDGE_SPACING.nest とセット）。
   */
  nestGap: { node: 8, rank: 10 },
}
